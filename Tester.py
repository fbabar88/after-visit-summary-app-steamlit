...
def main():
    ...
    # --- Sidebar Form for main patient details (excluding Medication Change) ---
    with st.sidebar.form(key="input_form"):
        ...
        with st.expander("Diabetes & Hypertension", expanded=False):
            bp_status = st.radio(
            ...
            if bp_status == "Above Goal":
                bp_reading = st.text_input(
                    "Enter BP Reading",
                    key="bp_reading", help="Enter blood pressure reading (e.g., 150/90)."
                )
            else:
                bp_reading = "At Goal"
            ...
            a1c_level = st.text_input(
                ...
                key="a1c", help="Enter the latest A1c level if available."
            )
        
        # Labs – Each lab category as its own expander
        with st.expander("Electrolytes", expanded=False):
            electrolytes_review = st.radio(
                "Review Electrolytes?", ["Not Reviewed", "Reviewed"],
        ...
        with st.expander("Anemia", expanded=False):
            anemia_review = st.radio(
                "Review Anemia?", ["Not Reviewed", "Reviewed"],
        ...
        with st.expander("Bone Mineral Disease", expanded=False):
            bone_review = st.radio(
                "Review Bone Mineral Disease?", ["Not Reviewed", "Reviewed"],
    ...
    # --- Interactive Medication Change & Follow-up (outside the form) ---
    st.sidebar.header("Medication Change & Follow-up")
    med_change = st.sidebar.radio(
        "Medication Change?", ["No", "Yes", "N/A"],
    ...
    if med_change == "Yes":
        med_change_types = st.sidebar.multiselect(
            ...
            key="med_change_list", help="Select the medications that have been changed."
        )
    else:
        med_change_types = []
    
    followup_appointment = st.sidebar.text_input(
    ...
    if submit_button:
        # If free text is provided (optional override), use that; otherwise, combine inputs.
        free_text_command = st.session_state.get("free_text", "")
